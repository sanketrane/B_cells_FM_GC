...
init <- function() list(
  psi = exp(rnorm(1, log(0.001), 0.01)),
  delta = exp(rnorm(1,log(0.5), 0.01)),
  deltaYFP = exp(rnorm(1, log(0.1), 0.01)),
  r_d = exp(rnorm(1, log(0.001), 0.001)),
...
initfun <- init()
lambda0 = -log(initfun$deltaYFP)/(58 * exp(-initfun$r_d * 58))
initfun$delta - lambda0


## Specify the variables for which you want history and density plots
parametersToPlot <- c("psi", "deltaYFP", "r_d", "rho0", "delta", "Beta",  "y0_Log", "kappa_0",
...
otherRVs <- c("y1_mean_pred_age1", "countspred_age1", "y1_mean_pred_age2", "countspred_age2", "y1_mean_pred_age3", "countspred_age3",
              ...
              "lambda_calc", "lambda_inv", "rho_inv",  "delta_inv", "r_d_inv",
