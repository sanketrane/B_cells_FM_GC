...
if (grepl("FM", data_derived1) == TRUE){
  modelName = "ki67_KHM_LNGC_FM"
} else {
  modelName <- "ki67_KHM_LNGC"        # name of the file for stan model# model sepcific details
}
...
data <- list(
  ...
  ki_donor = ki67_sorted$host_ki67_GC,
...
#initfun <- init()
#print((initfun$deltaYFP - exp(-initfun$lambdaSlow *58))/(exp(-initfun$lambdaFast * 58) - exp(-initfun$lambdaSlow * 58)))
...
otherRVs <- c("y1_mean_pred_age1", "countspred_age1", "y1_mean_pred_age2", "countspred_age2", "y1_mean_pred_age3", "countspred_age3",
  ...
  "log_lik","log_lik1", "log_lik2", "log_lik3", "log_lik4")
