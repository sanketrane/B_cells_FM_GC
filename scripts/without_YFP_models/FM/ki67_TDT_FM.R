## Time-dependent turnover model -- Homogeneous model 
...
## model files (Stan)
modelName <- "ki67_TDT_FM"        # name of the file for stan model 
...
seed <- 1040 + i                     ### Use your lucky mumber! haha! 
...
data <- list(
  ...
  ki_host = ki67_sorted$host_ki67_FM,        ## data to fit
  ki_donor = ki67_sorted$donor_ki67_FM,      ## data to fit
...
init <- function() list(
  psi = exp(rnorm(1, log(0.5), 0.1)),
  rho = exp(rnorm(1,log(0.01), 0.01)),
  delta0 = exp(rnorm(1,log(0.02), 0.01)),
  r_d = exp(rnorm(1, log(0.001), 0.01)),
  Beta = rnorm(1, 4, 0.1),
  
  y0_log = rnorm(1, 16, 1),
  kappa_0 = exp(rnorm(1, log(0.2), 0.1)),
...
parametersToPlot <- c("psi", "delta0", "r_d", "rho", "Beta",  "y0_Log", "kappa_0",
...
otherRVs <- c("y1_mean_pred_age1", "countspred_age1", "y1_mean_pred_age2", "countspred_age2", "y1_mean_pred_age3", "countspred_age3",
              ...
              "lambda0", "lambda_inv", "rho_inv", "delta_inv",
...
nChains <- 2
nPost <- 2500 ## Number of post-burn-in samples per chain after thinning
