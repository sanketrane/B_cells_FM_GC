## Time-dependent source influx model -- Homogeneous model 
...
## model files (Stan)
modelName <- "ki67_SVM_FM"        # name of the file for stan model 
...
init <- function() list(
  psi = exp(rnorm(1, log(0.1), 0.01)),
  rho = exp(rnorm(1,log(0.001), 0.01)),
  delta = exp(rnorm(1,log(0.005), 0.01)),
  r_psi = exp(rnorm(1,log(0.01), 0.01)),
  Beta = rnorm(1, 3.5, 0.01),
...
parametersToPlot <- c("psi", "rho", "delta", "r_psi",  "Beta", "y0_Log", "kappa_0",
