## Time-dependent division model -- Homogeneous model 
...
## model files (Stan)
modelName <- "ki67_TDD_FM"        # name of the file for stan model 
...
seed <- 25 + i                     ### Use your lucky mumber! haha! 
...
init <- function() list(
  psi = exp(rnorm(1, log(0.8), 0.1)),
  delta = exp(rnorm(1,log(0.03), 0.01)),
  rho0 = exp(rnorm(1,log(0.01), 0.01)),
...
parametersToPlot <- c("psi", "rho0", "r_d", "Beta",  "y0_Log", "kappa_0",
...
otherRVs <- c("y1_mean_pred_age1", "countspred_age1", "y1_mean_pred_age2", "countspred_age2", "y1_mean_pred_age3", "countspred_age3",
              ...
              "lambda_inv",  "lambda0", "rho_inv", "delta", "delta_inv",
