## Time-dependent Influx -- Homogeneous model 
...
## model files (Stan)
modelName <- "ki67_SVM_LNGC"        # name of the file for stan model 
...
init <- function() list(
  ...
  rho = exp(rnorm(1, log(0.01), 0.1)),
  delta = exp(rnorm(1, log(0.01), 0.1)),
  r_psi = exp(rnorm(1, log(0.001), 0.1)),
  
  y0_Log = rnorm(1, 11 , 0.05),
...
parametersToPlot <- c("psi", "rho", "r_psi", "delta",  "Beta", "y0_Log", "kappa_0", 
...
otherRVs <- c("y1_mean_pred_age1", "countspred_age1", "y1_mean_pred_age2", "countspred_age2", "y1_mean_pred_age3", "countspred_age3",
              ...
              "lambda_inv", "lambda", "rho_inv", "delta", "delta_inv",
