## Incumbent model -- heterogenous model 

## clearing the environment and freeing the memory held by previously loaded objects
rm(list = ls()); gc()   

# working directory where the folders for Stan models, Rscripts, and Data files are located
setwd("/opt/mesh/eigg/sanket/FM_GC_ageCorrected")

#### this trick is to use the same R script for running models that only differ in single variable e.g. here we are varying the precursor population
#### calling this script from command line will need at least one argument specified in front of it (name of the csv file for the precursor pop)
...
## model files (Stan)
modelName <- "ki67_INC_SPGC"        # name of the file for stan model 
#### in some cases name of the stan file is depedent on the name of the precursor pop used

## Setting all the directories for opeartions
projectDir <- getwd()
scriptDir <- file.path(projectDir, "scripts", "without_YFP_models", "SPGC")
modelDir <- file.path(projectDir, "models", "without_YFP_models", "SPGC")
dataDir <- file.path(projectDir, "data")
toolsDir <- file.path(scriptDir, "tools")
outputDir <- file.path(projectDir, "output", "without_YFP_models", "SPGC")
...
init <- function() list(
  psi = exp(rnorm(1, log(0.001), 0.5)),
  rho = exp(rnorm(1,log(0.01), 0.5)),
  lambda = exp(rnorm(1,log(0.01), 0.01)),
  rho_inc = exp(rnorm(1,log(0.01), 0.5)),
  alpha = exp(rnorm(1, log(0.1), 0.01)),
  
  y0_Log = rnorm(1, 11 , 0.1),
  kappa_0 = exp(rnorm(1, log(0.9), 0.01)),
  kappa_inc = exp(rnorm(1, log(0.9), 0.01)),
...
parametersToPlot <- c("psi", "delta", "rho", "lambda", "rho_inc",  "Beta", "alpha",
                      "y0_Log", "kappa_0", "kappa_inc",
...
otherRVs <- c("y1_mean_pred_age1", "countspred_age1", "y1_mean_pred_age2", "countspred_age2", "y1_mean_pred_age3", "countspred_age3",
              ...
              "lambda_inv", "rho_inv", "delta_inv", "rhoInc_inv",
...
loo_loglik2 <- extract_log_lik(fit, parameter_name = "log_lik2", merge_chains = TRUE)
loo_loglik3 <- extract_log_lik(fit, parameter_name = "log_lik3", merge_chains = TRUE)
loo_loglik4 <- extract_log_lik(fit, parameter_name = "log_lik4", merge_chains = TRUE)

loo_ic <- loo(loo_loglik1, loo_loglik2, loo_loglik3, loo_loglik4)
