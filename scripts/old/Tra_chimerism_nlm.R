...
ggplot() +
  ...
  geom_ribbon(data = Ypred, aes(x = timeseries, ymin = lb, ymax = ub), fill = "#f23047", alpha = 0.35)+
  geom_line(data = Ypred, aes(x = timeseries, y = median), size=1.5) +
  geom_point(data = data_imp, aes(x = days.post.bmt, y = fd), size=3) +
  labs(x = "Days post t0", y = NULL, title = "Normalised Donor fractions: T2") +
  scale_x_continuous(limits = c(0, 500), breaks = c(0,150,300,450))+
  scale_y_continuous(limits =c(-0.15, 1.25), breaks = c(0, 0.3, 0.6, 0.9, 1.2, 1.5))+ 
