...
modelName <- "T1_counts_linear"
data_derived <- "T1_data.csv"
...
parametersToPlot <- c("theta0","nu","sigma")
...
pred <- as.data.frame(fit, pars = "countspred") %>%
...
## my style
ggplot()+
  geom_ribbon(data = pred, aes(x = ts_pred, ymin = lb, ymax = ub), fill = "#ff591c", alpha = 0.35)+
  geom_line(data = pred, aes(x = ts_pred, y = median), col = '#ff591c', size=1.5) +
  geom_point(data = data_imp, aes(x = age.at.S1K, y = total_counts), size=3) +
  scale_color_manual(values = c("#0099cc", "#ff591c"))+
  labs(x = "Host age (days)", y = NULL, title = "Total counts T1 cells") +
  scale_x_continuous(limits = c(70, 600), trans = "log10",  breaks = c(10, 30, 100, 300))+
  scale_y_continuous(limits =c(5e4, 1e7), trans = "log10", breaks = c(1e6, 1e7, 1e8, 1e5), minor_breaks = log10minorbreaks, labels =fancy_scientific)+ 
