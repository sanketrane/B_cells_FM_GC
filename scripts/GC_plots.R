...
filter(timeseries >= 60) %>%  filter(timeseries <= 475)
...
Cpred2 <- as.data.frame(fit, pars = "countspred_age2") %>%
  ...
  filter(timeseries >= 75) %>%  filter(timeseries <= 330)
...
Cpred3 <- as.data.frame(fit, pars = "countspred_age3") %>%
  ...
  filter(timeseries >= 220) %>%  filter(timeseries <= 475)
...
fdpred1 <- as.data.frame(fit, pars = "fdpred_age1") %>%
    ...
    bind_cols("timeseries" = ts_pred1,
              "age_bins" = rep("Age bin#1: <8Wks", length(ts_pred1))) %>%
  filter(timeseries >= Mean_agebin1) %>%  
  filter(timeseries <= 475)
  ...
  fdpred2 <- as.data.frame(fit, pars = "fdpred_age2") %>%
    ...
    bind_cols("timeseries" = ts_pred2,
              "age_bins" = rep("Age bin#2: 8-12Wks", length(ts_pred2))) %>%
    filter(timeseries >= Mean_agebin2) %>%  
    filter(timeseries <= 475)
  ...
  fdpred3 <- as.data.frame(fit, pars = "fdpred_age3") %>%
    ...
    summarize(lb = quantile(value, probs = 0.045),
              median = quantile(value, probs = 0.5),
              ub = quantile(value, probs = 0.955)) %>%
    bind_cols("timeseries" = ts_pred3 + 20,
              "age_bins" = rep("Age bin#3: >12Wks", length(ts_pred3))) %>%
    filter(timeseries >= Mean_agebin3) %>% 
    filter(timeseries <= 475)
  ...
  donor_kiPred1 <- as.data.frame(fit, pars = "donor_kiprop_pred1") %>%
    ...
    bind_cols("timeseries" = ts_pred1,
              "age_bins" = rep("Age bin#1: <8Wks", length(ts_pred1)))%>%
    filter(timeseries >= 70) %>%  filter(timeseries <= 475)
  ...
  donor_kiPred2 <- as.data.frame(fit, pars = "donor_kiprop_pred2") %>%
    ...
    bind_cols("timeseries" = ts_pred2,
              "age_bins" = rep("Age bin#2: 8-12Wks", length(ts_pred2)))%>%
    filter(timeseries >= 73) %>%  filter(timeseries <= 475)
  ...
  ki67_facet <- ggplot() +
    ...
    myTheme + theme(legend.position = c(0.92, 0.2), legend.background = element_blank(), strip.background = element_blank())
 
  #### combined plot for cell counts
  counts_facet <- ggplot() +
    ...
    scale_x_continuous(limits = c(60, 500), trans = "log10", breaks = c(75, 150, 300, 500))+
    scale_y_continuous(limits = c(1e3, 5e6), trans="log10", breaks=c(1e4, 1e5, 1e6, 1e7, 1e8), minor_breaks = log10minorbreaks, labels =fancy_scientific) +
  ...
  nfd_facet <- ggplot() +
    ...
    scale_fill_manual(values=c("#3B9AB2", "#E1AF00", "#F21A00"), name = NULL,
                      labels = c("<8Wks", "8-12Wks", ">12Wks"))+
    guides(fill = FALSE) + myTheme + theme(legend.position = c(0.85, 0.2), legend.spacing = unit(0.2, "cm") , legend.key.size = unit(0.5, "cm"))
  ...
  nfd_comb <- nfd_facet +
    ...
    scale_x_continuous(limits = c(40, 475), breaks = c(50, 150, 250, 350, 450))+
    scale_y_continuous(limits =c(0, 1.2), breaks = c(0, 0.25, 0.5, 0.75, 1.0)) 
  ...
  ki67_comb <- ggplot() +
    ...
    theme(legend.position = c(0.72, 0.2), legend.box = "horizontal", legend.spacing = unit(0.2, "cm") , legend.key.size = unit(0.5, "cm")) + guides(fill = FALSE) 
  ...
  nfd_plot <- nfd_facet +
    ...
    labs(x = "Host age (days)", y = NULL, title = paste("Normalised chimerism within", substr(modelName, 10, 13), "B cells")) +
    scale_x_continuous(limits = c(40, 475), breaks = c(50, 150, 250, 350, 450))+
    scale_y_continuous(limits =c(0, 1.3), breaks = c(0, 0.25, 0.5, 0.75, 1.0)) + guides(color = FALSE) +
    facet_wrap(~ age_bins)
