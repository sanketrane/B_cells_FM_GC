functions{
  ...
  real[,] solve_ode(real[] solve_time, real[] init_cond, real[] parms, real[] rdata, int[] tb, int num_index, real[] tb_time, int[] tb_index, int num_tb){
   ...
   init_tb[2] = init_cond[2];

   for (i in 1:num_index){
     init_tb[3] = y0[tb_index[i], 3];
     init_tb[4] = y0[tb_index[i], 4];
     y_hat[i] = foreach_ode(solve_time[i], tb[tb_index[i]], init_tb, parms, rdata, tb[tb_index[i]]);
   }
