...
parameters{
  ...
  real<lower = 0, upper = (-log(deltaYFP)/58)> lambdaSlow;
  ...
  transformed parameters{
    ...
    alpha_calc = (deltaYFP - exp(-lambdaSlow * 58)) /(exp(-lambdaFast * 58) - exp(-lambdaSlow * 58));

    init_cond[1] = Nd_0;
    init_cond[2] = Nd_0;
    init_cond[3] = Nd_0;
    init_cond[4] = Nd_0;
    init_cond[5] = y0 * alpha_calc * kappaF_0;
    init_cond[6] = y0 * alpha_calc * (1- kappaF_0);
    init_cond[7] = y0 * (1-alpha_calc) * kappaS_0;
    init_cond[8] = y0 * (1-alpha_calc) * (1- kappaS_0);
...
model{
  ...
  f_fast ~ normal(0.6, 0.25);
  rhoFast ~ normal(0.1, 0.5);
  rhoSlow ~ normal(0.01, 0.5);
  lambdaFast ~ normal(0.1, 0.5);
  lambdaSlow ~ normal(0.001, 0.5);
  deltaYFP ~ skew_normal(0.42, 0.1, -5);
  Beta ~ normal(3.5, 1);

  y0_Log ~ normal(10, 2);
  kappaF_0 ~ normal(0.8, 0.05);
  kappaS_0 ~ normal(0.8, 0.05);
...
generated quantities{
  ...
  y4_mean_pred1[1] = (y0 * alpha_calc * kappaF_0 + y0 * (1-alpha_calc) * kappaS_0)/(y0);
  host_kiprop_pred1[1] = expit_boundary(normal_rng(logit_boundary(y4_mean_pred1[1]), sigma4));
  y4_mean_pred2[1] = (y0 * alpha_calc * kappaF_0 + y0 * (1-alpha_calc) * kappaS_0)/(y0);
  host_kiprop_pred2[1] = expit_boundary(normal_rng(logit_boundary(y4_mean_pred2[1]), sigma4));
  y4_mean_pred3[1] = (y0 * alpha_calc * kappaF_0 + y0 * (1-alpha_calc) * kappaS_0)/(y0);
