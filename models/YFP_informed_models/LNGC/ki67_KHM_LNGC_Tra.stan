...
parameters{
  ...
  real<lower = 0> rhoFast;
  real<lower = 0> rhoSlow;
  real lambdaFast;
  real<upper = lambdaFast>  lambdaSlow;
...
model{
  ...
  f_fast ~ uniform(0, 1);
  rhoFast ~ normal(1, 0.5);
  rhoSlow ~ normal(1, 0.5);
  lambdaSlow ~ normal(0.001, 0.5);
  lambdaFast ~ normal(0.005, 0.5);
  deltaYFP ~ normal(0.25, 0.03);
  Beta ~ normal(3.5, 1);

  y0_Log ~ normal(10, 2);
  //alpha_calc ~ normal(0.5, 0.25);
