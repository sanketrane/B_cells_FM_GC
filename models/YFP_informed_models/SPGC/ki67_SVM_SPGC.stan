functions{
  real theta_spline(real t,
   ...
   real Chi_spline( real t,
    ...
    real[] shm(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
      ...
      dkdt[1] = ((psi * exp(r_psi * (t-ta))) * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * eps_donor) + rho * (2 * k[2] + k[1]) - ((1/Beta) + delta) * k[1];
      dkdt[2] = ((psi * exp(r_psi * (t-ta))) * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * (1-eps_donor)) + (1/Beta) * k[1] - (rho + delta) * k[2];

      dkdt[3] = ((psi * exp(r_psi * (t-ta))) * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * eps_host) + rho * (2 * k[4] + k[3]) - ((1/Beta) + delta) * k[3];
      dkdt[4] = ((psi * exp(r_psi * (t-ta))) * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * (1-eps_host)) + (1/Beta) * k[3] - (rho + delta) * k[4];
  ...
  int ta  = 40;                                                        // the earliest tbmt

  y0 = solve_init(tb_time, init_cond, parms, rdata, ta, num_tb);
...
parameters{
  ...
  real<lower = 0, upper=1> p_Influx;
  real<lower = 0> rho;
  real<lower = 0, upper =1>  deltaYFP;
  real<lower = 0> Beta;
  real<lower = -0.03, upper = 0.03> r_psi;
...
transformed parameters{
  ...
  real y0 = exp(y0_Log);
  real lambda_calc = -log(deltaYFP)/ 58;
  real delta = lambda_calc + rho;

  real psi = (y0 * p_Influx)/ theta0;
...
model{
  p_Influx ~ normal(0.3, 0.2);
  rho ~ normal(0.1, 0.5);
  Beta ~ normal(3.5, 1);
  deltaYFP ~ normal(0.13, 0.03);
  r_psi ~ normal(0.0, 0.05);
...
generated quantities{
 ...
 real rho_inv = 1/rho;
 real lambda =  delta - rho;
 real lambda_inv = 1/lambda;
 real r_psi_inv = log(2)/r_psi;
