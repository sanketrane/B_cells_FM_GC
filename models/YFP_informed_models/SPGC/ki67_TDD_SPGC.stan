functions{
  ...
  real[] tdd(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
     real psi     = parms[1];
     real delta   = parms[2];
     real rho0    = parms[3];
     real Beta    = parms[4];
     ...
     dkdt[1] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * eps_donor) + rho0 * exp(-r_d * (t-ta)) * (2 * k[2] + k[1]) - ((1/Beta) + delta) * k[1];
     dkdt[2] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * (1-eps_donor)) + (1/Beta) * k[1] - (rho0 * exp(-r_d * (t-ta)) + delta) * k[2];

     dkdt[3] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * eps_host) + rho0 * exp(-r_d * (t-ta)) * (2 * k[4] + k[3]) - ((1/Beta) + delta) * k[3];
     dkdt[4] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * (1-eps_host)) + (1/Beta) * k[3] - (rho0 * exp(-r_d * (t-ta)) + delta) * k[4];
   ...
   real[] foreach_init(real tb, real ta, real[] init_cond, real[] parms, real[] rdata, int x_i){
     ...
     return to_array_1d(integrate_ode_rk45(tdd, init_cond, ta, rep_array(tb, 1), parms, rdata, rep_array(x_i, 1)));
   }
   ...
   real[] foreach_ode(real ts, real t0, real[] init_cond, real[] parms, real[] rdata, int x_i) {
     ...
     return to_array_1d(integrate_ode_rk45(tdd, init_cond, t0, rep_array(ts, 1), parms, rdata, rep_array(x_i, 1)));
    }
...
parameters{
  ...
  real<lower = 0> delta;
  real<lower = -0.03, upper = 0.03> r_d;
...
transformed parameters{
  ...
  real y0 = exp(y0_Log);
  real <lower = 0> rho0 = (delta * 58 + log(deltaYFP))/(58 * exp(-r_d * 58));
  real lambda_calc = delta - rho0;
...
generated quantities{
  ...
  for (i in 2:numPred1){
    ...
    donor_kiprop_pred1[i] =  expit(normal_rng(logit_boundary(y3_mean_pred1[i]), sigma3));

    y4_mean_pred1[i] = k_hat_pred_age1[i, 3]/(k_hat_pred_age1[i, 3] + k_hat_pred_age1[i, 4]);
    host_kiprop_pred1[i] = expit(normal_rng(logit_boundary(y4_mean_pred1[i]), sigma4));
  }
  ...
  for (i in 2:numPred2){
    ...
    donor_kiprop_pred2[i] =  expit(normal_rng(logit_boundary(y3_mean_pred2[i]), sigma3));

    y4_mean_pred2[i] = k_hat_pred_age2[i, 3]/(k_hat_pred_age2[i, 3] + k_hat_pred_age2[i, 4]);
    host_kiprop_pred2[i] = expit(normal_rng(logit_boundary(y4_mean_pred2[i]), sigma4));
  }
  ...
  for (i in 2:numPred3){
    ...
    donor_kiprop_pred3[i] =  expit(normal_rng(logit_boundary(y3_mean_pred3[i]), sigma3));

    y4_mean_pred3[i] = k_hat_pred_age3[i, 3]/(k_hat_pred_age3[i, 3] + k_hat_pred_age3[i, 4]);
    host_kiprop_pred3[i] = expit(normal_rng(logit_boundary(y4_mean_pred3[i]), sigma4));
  }
