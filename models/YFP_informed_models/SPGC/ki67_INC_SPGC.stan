functions{
  ...
  real[] INC(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
    real psi       = parms[1];
    real rho       = parms[2];
    real rho_inc   = parms[3];
    real Beta      = parms[4];
    real delta     = parms[5];
    ...
    real dkdt[6];
     ...
     // ki67hi donor
     dkdt[1] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * eps_donor) + rho * (2 * k[2] + k[1]) - ((1/Beta) + delta) * k[1];
     // ki67lo donor
     dkdt[2] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * (1-eps_donor)) + (1/Beta) * k[1] - (rho + delta) * k[2];

     // ki67hi host
     dkdt[3] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * eps_host) + rho * (2 * k[4] + k[3]) - ((1/Beta) + delta) * k[3];
     //ki67lo host
     dkdt[4] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * (1-eps_host)) + (1/Beta) * k[3] - (rho + delta) * k[4];
     //ki67hi Incumbent
     dkdt[5] = rho_inc * (2 * k[6] + k[5]) - ((1/Beta) + rho_inc) * k[5];
     //ki67lo Incumbent
     dkdt[6] = (1/Beta) * k[5] - (rho_inc + rho_inc) * k[6];
  ...
  real[] foreach_ode(real ts, real t0, real[] init_cond, real[] parms, real[] rdata, int x_i) {
   ...
   real[,] solve_ode(real[] solve_time, real[] init_cond, real[] parms, real[] rdata, int[] tb, int num_index, real[] tb_time, int[] tb_index, int num_tb){
    ...
    for (i in 1:num_index){
      init_tb[3] = y0[tb_index[i], 1] + y0[tb_index[i], 3];              //at tbmt - all dispaceable ki67Hi cells would be host-derived
      init_tb[4] = y0[tb_index[i], 2] + y0[tb_index[i], 4];              //at tbmt - all dispaceable ki67Lo cells would be host-derived
      init_tb[5] = y0[tb_index[i], 5];                                    //at tbmt - ki67Hi incumbent compartment in host-derived
      init_tb[6] = y0[tb_index[i], 6];                                   //at tbmt - ki67Lo incumbent compartment in host-derived
   ...
   real[,] solve_ode_pred(real[] solve_time, real[] init_cond, real[] parms, real[] rdata, int[] tb, int num_index, real[] tb_time){
    ...
    init_tb[3] = y0[2, 1] + y0[2, 3];                                   //at tbmt - all ki67Hi cells would be host-derived
    init_tb[4] = y0[2, 2] + y0[2, 4];                                   //at tbmt - all ki67Lo cells would be host-derived
    init_tb[5] = y0[2, 5];                                               //at tbmt - ki67Hi incumbent compartment in host
    init_tb[6] = y0[2, 6];                                               //at tbmt - ki67Lo incumbent compartment in host
  ...
  real expit_boundary_boundary(real x){
    real answer;
    real b = 1.5;
...
transformed data{
  ...
  y2 = logit_boundary_array(Nfd);                       // logit transfored cell counts of donor fractions normalised to source chimerism to feed in to ODEs
...
parameters{
  ...
  real<lower = 0, upper = 1> kappa_inc;
  real<lower = 0, upper = 1> psi;
  real<lower = 0> rho;
  real<lower = 0> rho_inc;
  real<lower = 0, upper =1>  deltaYFP;
  real<lower = (-log(deltaYFP)/58)> lambda;
  real<lower = 0> Beta;
...
transformed parameters{
  ...
  real<lower = 0> delta = lambda + rho;
  alpha_calc = (deltaYFP - exp(-lambda * 58)) /(1 - exp(-lambda * 58));

  init_cond[1] = Nd_0;
  init_cond[2] = Nd_0;
  init_cond[3] = y0 * (1 - alpha_calc) * kappa_0;
  init_cond[4] = y0 * (1 - alpha_calc) * (1 - kappa_0);
  init_cond[5] = y0 * alpha_calc * kappa_inc;
  init_cond[6] = y0 * alpha_calc * (1 - kappa_inc);
  ...
  for (i in 1:numObs){
    // total counts
    y1_mean[i] = k_hat[time_index[i], 1] + k_hat[time_index[i], 2] + k_hat[time_index[i], 3] + k_hat[time_index[i], 4] + k_hat[time_index[i], 5] + k_hat[time_index[i], 6] ;
    ...
    y4_mean[i] = (k_hat[time_index[i], 3] + k_hat[time_index[i], 5])/(k_hat[time_index[i], 3] + k_hat[time_index[i], 4] + k_hat[time_index[i], 5] + k_hat[time_index[i], 6]);
  }
}
...
model{
  psi ~ normal(0.01, 0.5);
  rho ~ normal(0.1, 0.5);
  rho_inc ~ normal(0.1, 0.5);
  lambda ~ normal(0.05, 0.5);
  deltaYFP ~ normal(0.1, 0.03);
  Beta ~ normal(3.5, 1);

  y0_Log ~ normal(10, 2);
  kappa_0 ~  normal(0.9, 0.05);
  kappa_inc ~ normal(0.05, 0.5);

  sigma1 ~ normal(0.5, 0.5);
  sigma2 ~ normal(0.07, 0.5);
  ...
  real k_hat_pred_age1[numPred1, 6];
  real k_hat_pred_age2[numPred2, 6];
  real k_hat_pred_age3[numPred3, 6];
  ...
  for (i in 1:numPred1){
    y1_mean_pred_age1[i] = k_hat_pred_age1[i, 1] + k_hat_pred_age1[i, 2] + k_hat_pred_age1[i, 3] + k_hat_pred_age1[i, 4] +
     k_hat_pred_age1[i, 5] + k_hat_pred_age1[i, 6];
  ...
  for (i in 1: numPred2){
    y1_mean_pred_age2[i] = k_hat_pred_age2[i, 1] + k_hat_pred_age2[i, 2] + k_hat_pred_age2[i, 3] + k_hat_pred_age2[i, 4] +
     k_hat_pred_age2[i, 5] + k_hat_pred_age2[i, 6];
  ...
  for (i in 1: numPred3){
    y1_mean_pred_age3[i] = k_hat_pred_age3[i, 1] + k_hat_pred_age3[i, 2] + k_hat_pred_age3[i, 3] + k_hat_pred_age3[i, 4] +
     k_hat_pred_age3[i, 5] + k_hat_pred_age3[i, 6];
  ...
  y4_mean_pred1[1] = ((y0 * (1-alpha_calc) * kappa_0) + (y0 * alpha_calc * kappa_inc))/(y0);
  host_kiprop_pred1[1] = expit_boundary(normal_rng(logit_boundary(y4_mean_pred1[1]), sigma4));
  y4_mean_pred2[1] = ((y0 * (1-alpha_calc) * kappa_0) + (y0 * alpha_calc * kappa_inc))/(y0);
  host_kiprop_pred2[1] = expit_boundary(normal_rng(logit_boundary(y4_mean_pred2[1]), sigma4));
  y4_mean_pred3[1] = ((y0 * (1-alpha_calc) * kappa_0) + (y0 * alpha_calc * kappa_inc))/(y0);
  ...
  for (i in 2:numPred1){
    ...
    y4_mean_pred1[i] = (k_hat_pred_age1[i, 3] + k_hat_pred_age1[i, 5])/(k_hat_pred_age1[i, 3] + k_hat_pred_age1[i, 4] + k_hat_pred_age1[i, 5] + k_hat_pred_age1[i, 6]);
  ...
  for (i in 2:numPred2){
    ...
    y4_mean_pred2[i] = (k_hat_pred_age2[i, 3] + k_hat_pred_age2[i, 5])/(k_hat_pred_age2[i, 3] + k_hat_pred_age2[i, 4] + k_hat_pred_age2[i, 5] + k_hat_pred_age2[i, 6]);
  ...
  for (i in 2:numPred3){
    ...
    y4_mean_pred3[i] = (k_hat_pred_age3[i, 3] + k_hat_pred_age3[i, 5])/(k_hat_pred_age3[i, 3] + k_hat_pred_age3[i, 4] + k_hat_pred_age3[i, 5] + k_hat_pred_age3[i, 6]);
