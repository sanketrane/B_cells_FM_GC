functions{
  ...
  real[] tdd(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
     ...
     dkdt[1] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * eps_donor) + (delta - (lambda0 * exp(-r_d * (t-ta)))) * (2 * k[2] + k[1]) - ((1/Beta) + delta) * k[1];
     dkdt[2] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * (1-eps_donor)) + (1/Beta) * k[1] - ((delta - (lambda0 * exp(-r_d * (t-ta)))) + delta) * k[2];

     dkdt[3] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * eps_host) + (delta - (lambda0 * exp(-r_d * (t-ta)))) * (2 * k[4] + k[3]) - ((1/Beta) + delta) * k[3];
     dkdt[4] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * (1-eps_host)) + (1/Beta) * k[3] - ((delta - (lambda0 * exp(-r_d * (t-ta)))) + delta) * k[4];
