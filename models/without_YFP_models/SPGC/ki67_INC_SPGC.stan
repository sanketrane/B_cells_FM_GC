...
transformed parameters{
  ...
  init_cond[1] = Nd_0;
  init_cond[2] = Nd_0;
  init_cond[3] = y0 * (1 - alpha) * kappa_0;
  init_cond[4] = y0 * (1 - alpha) * (1 - kappa_0);
  init_cond[5] = y0 * alpha * kappa_inc;
  init_cond[6] = y0 * alpha * (1 - kappa_inc);
...
generated quantities{
 ...
 y4_mean_pred1[1] = ((y0 * (1-alpha) * kappa_0) + (y0 * alpha * kappa_inc))/(y0);
 host_kiprop_pred1[1] = expit_boundary(normal_rng(logit_boundary(y4_mean_pred1[1]), sigma4));
 y4_mean_pred2[1] = ((y0 * (1-alpha) * kappa_0) + (y0 * alpha * kappa_inc))/(y0);
 host_kiprop_pred2[1] = expit_boundary(normal_rng(logit_boundary(y4_mean_pred2[1]), sigma4));
 y4_mean_pred3[1] = ((y0 * (1-alpha) * kappa_0) + (y0 * alpha * kappa_inc))/(y0);
