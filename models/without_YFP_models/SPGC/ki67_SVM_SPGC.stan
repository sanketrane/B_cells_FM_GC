  ...
  real theta_spline(real t,
   ...
   real Chi_spline( real t,
    ...
    real[] shm(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
      ...
      dkdt[1] = ((psi * exp(r_psi * t)) * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * eps_donor) + rho * (2 * k[2] + k[1]) - ((1/Beta) + delta) * k[1];
      dkdt[2] = ((psi * exp(r_psi * t)) * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * (1-eps_donor)) + (1/Beta) * k[1] - (rho + delta) * k[2];

      dkdt[3] = ((psi * exp(r_psi * t)) * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * eps_host) + rho * (2 * k[4] + k[3]) - ((1/Beta) + delta) * k[3];
      dkdt[4] = ((psi * exp(r_psi * t)) * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * (1-eps_host)) + (1/Beta) * k[3] - (rho + delta) * k[4];
...
transformed parameters{
  ...
  real y0 = exp(y0_Log);
  //real lambda_calc = -log(deltaYFP)/ 41;
