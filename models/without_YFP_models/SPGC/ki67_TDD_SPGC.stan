functions{
  ...
  real[] tdd(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
   ...
   real[] foreach_init(real tb, real ta, real[] init_cond, real[] parms, real[] rdata, int x_i){
     ...
     return to_array_1d(integrate_ode_bdf(tdd, init_cond, ta, rep_array(tb, 1), parms, rdata, rep_array(x_i, 1)));
   }
   ...
   real[] foreach_ode(real ts, real t0, real[] init_cond, real[] parms, real[] rdata, int x_i) {
     ...
     return to_array_1d(integrate_ode_bdf(tdd, init_cond, t0, rep_array(ts, 1), parms, rdata, rep_array(x_i, 1)));
    }
...
transformed parameters{
  ...
  real init_cond[4];
  real y0 = exp(y0_Log);
  //real lambda_calc = -log(deltaYFP)/(41 * exp(-r_d * 41));
...
model{
  ...
  real lambda0 = delta - rho0;
  real lambda_inv = 1/lambda0;
  real rho_inv = 1/rho0;
