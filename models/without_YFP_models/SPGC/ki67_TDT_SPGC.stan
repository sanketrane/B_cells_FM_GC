functions{
  ...
  real[] tdt(real t,  real[] k, real[] parms, real[] rdata, int[] idata) {
     ...
     dkdt[1] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * eps_donor) + rho * (2 * k[2] + k[1]) - ((1/Beta) + (delta0 * exp(-r_d * (t-ta)))) * k[1];
     dkdt[2] = (psi * theta_spline(t, nu, theta0) * Chi_spline(t-tb, chiEst, qEst) * (1-eps_donor)) + (1/Beta) * k[1] - (rho + (delta0 * exp(-r_d * (t-ta)))) * k[2];

     dkdt[3] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * eps_host) + rho * (2 * k[4] + k[3]) - ((1/Beta) + (delta0 * exp(-r_d * (t-ta)))) * k[3];
     dkdt[4] = (psi * theta_spline(t, nu, theta0) * (1-Chi_spline(t-tb, chiEst, qEst)) * (1-eps_host)) + (1/Beta) * k[3] - (rho + (delta0 * exp(-r_d * (t-ta)))) * k[4];
...
generated quantities{
  ...
  real delta_inv = 1/delta0;
  real rho_inv = 1/rho;
  real lambda0 = delta0 - rho;
